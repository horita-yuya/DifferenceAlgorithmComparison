...
struct Wu<E: Equatable> {
    ...
    static func diff(from fromArray: [E], to toArray: [E]) -> [GraphScript] {
...
private extension Wu {
    ...
    static func reverseTree(path: [Edge], sinkVertice: Vertice) -> [GraphScript] {
        var nextToVertice = sinkVertice
        var scripts: [GraphScript] = []
        
        path.reversed().forEach { _, fromVertice, toVertice, script, snakeCount in
    ...
    static func exploreEditGraph(from fromArray: [E], to toArray: [E], isInversed: Bool = false) -> [Edge] {
        let fromCount = fromArray.count
        let toCount = toArray.count
        ...
        let edgeMove: (Int) -> (_y: Int, fromVertice: Vertice, toVertice: Vertice, script: GraphScript) = { k in
            ...
            if furthestReaching[index - 1] + 1 <= furthestReaching[index + 1] {
                ...
                let inversedScript: GraphScript = _y == 0 ? .insertToHead(from: _y - k - 1) : .insert(from: _y - k - 1, to: _y - 1)
                let defaultScript: GraphScript = .delete(at: _y - k - 1)
                script = isInversed ? inversedScript : defaultScript
            } else {
                _y = furthestReaching[index - 1] + 1
                fromVertice = .vertice(x: _y - k, y: _y - 1)
                toVertice = .vertice(x: _y - k, y: _y)
                let inversedScript: GraphScript = .delete(at: _y - 1)
                let defaultScript: GraphScript = _y - k == 0 ? .insertToHead(from: _y - 1) : .insert(from: _y - 1, to: _y - k - 1)
                script = isInversed ? inversedScript : defaultScript
            }
        ...
        let headForFurthest: (Int, Int) -> Void = { k, p in
            let index = k + fromCount
            let (_y, fromVertice, toVertice, script) = edgeMove(k)
        ...
        let sourceSnaking: () -> Void = {
            furthestReaching[fromCount] = snake(0, 0)
            path.append((P: 0,
                         from: .vertice(x: 0, y: 0),
                         to: .vertice(x: furthestReaching[fromCount], y: furthestReaching[fromCount]),
                         script: .sourceScript,
                         snakeCount: furthestReaching[fromCount])
            )
        }
